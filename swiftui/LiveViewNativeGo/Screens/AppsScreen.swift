...
struct AppsScreen: View {
                ...
                Section {
                    ...
                    ForEach(settings.recentURLs.reversed(), id: \.self) { url in
                        Button {
                            ...
                            Label {
                                Text(
                                    (url as NSURL).resourceSpecifier.flatMap({
                                        String($0.dropFirst(2))
                                    }) ?? url.absoluteString)
                            } icon: {
                                AsyncImage(url: url.replacing(path: "/apple-touch-icon.png")) {
                                    phase in
                                    switch phase {
                                    case .success(let image):
                                        image
                                            .resizable()
                                    case .failure:
                                        AsyncImage(url: url.replacing(path: "/favicon.ico")) {
                                            phase in
                                            switch phase {
                                            case .success(let image):
                                                image
    ...
    func appendingScheme(to inputURL: String) -> String {
        inputURL.starts(with: "http://") || inputURL.starts(with: "https://")
